...
//	r = engine->RegisterObjectMethod("string", "string &opAddAssign(const string &in)", asMETHODPR(string, operator+=, (const string&), string&), asCALL_THISCALL); assert( r >= 0 );
	...
	if (engine->GetTypeInfoByName("addon_callable_length")) {
		r = engine->RegisterObjectMethod("string", "addon_callable_length get_length() const property", asFUNCTION(StringLength), asCALL_CDECL_OBJLAST); assert( r >= 0 );
	} else {
...
#if AS_USE_STLNAMES != 1
	if (engine->GetTypeInfoByName("addon_callable_length")) {
		r = engine->RegisterObjectMethod("string", "addon_callable_length get_length() const property", asFUNCTION(StringLengthGeneric), asCALL_GENERIC); assert( r >= 0 );
	} else {
#if AS_USE_ACCESSORS != 1
